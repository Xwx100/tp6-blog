...
create database if not exists `%pre%db_name`;
use `%pre%db_name`;
...
create table if not exists `%pre%user_%group` (
    `%user_id` bigint unsigned NOT NULL,
    `%group_id` bigint unsigned NOT NULL,
    unique key (`%user_id`, `%group_id`)
) ENGINE=Innodb DEFAULT CHARSET=utf8 COMMENT='用户-组' ;

create table if not exists `%pre%group` (
...
create table if not exists `%pre%group_%role` (
    `%group_id` bigint unsigned NOT NULL,
...
create table if not exists `%pre%role_%menu` (
    `%role_id` bigint unsigned NOT NULL,
